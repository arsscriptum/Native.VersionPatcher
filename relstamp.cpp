...
void VerCallback1::callback( PCWSTR name, PCWSTR value )
{
	...
	// Filter off FileVersion, ProductVersion. always take from FIXED_INFO.
	if ( 0 == _wcsicmp(name, L"FileVersion") ) {
		// Optional tail: "1.2.3.4-tail" or "1.2.3.4 tail"
        size_t n = wcscspn(value, L" -");
        PCWSTR tail = n < wcslen(value) ? &value[n] : NULL;
		if ( tail ) {
            m_vd->cFileVerTailSeparator = (char)*tail++;
			while( *tail == _T(' ') ) tail++;
	...
	if ( 0 == _wcsicmp(name, L"ProductVersion") ) {
        ...
        PCWSTR tail = n < wcslen(value) ? &value[n] : NULL;
		if ( tail ) {
            m_vd->cProductVerTailSeparator = (char)*tail++;
			while( *tail == _T(' ') ) tail++;
    ...
    // Language and encoding of strings in version resource:
    if ( 0 == _wcsicmp(name, L"@LANG") ) {
        UINT32 langcp = (UINT32)wcstoul( value, NULL, 16 );
        unsigned langid = HIWORD(langcp); // high 16 bytes: LANGID, low 16 bytes: codepage
        m_vd->langid = langid;
        ...
        {
            // Language combinations not tested. It may work or not.
            dprint("Warning: Resource language unknown (lang %u sublang %u)\n", PRIMARYLANGID(langid), SUBLANGID(langid));
        }
...
BOOL fillFileInfo( __out file_ver_data_s *fvd, PCTSTR fpath )
{
	...
	if ( g_params.PatchMode ) 
	{
		...
		if (fxi->dwFileOS != VOS_NT_WINDOWS32 && fxi->dwFileOS != VOS__WINDOWS32 ) {
            ...
            // in some portable .NET executables dwFileOS == 0
		}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if ( !g_params.PatchMode ) { // if not patch mode, read from config.
...
// Aliases for ProductVersion are not here.
LPCTSTR aliasToStringKey( LPCTSTR key )
{
    ...
    if ( 0 == _tcsicmp(key, _T("private"))) return _T("PrivateBuild");
    ...
    // Assume it is a custom key and return as is
	return key;
}
	...
	__out_opt struct file_ver_data_s *fvd ) 
{
	...
	for( int i = 1; i < argc; i++ ) {
		...
		if ( *ap == _T('/') || *ap == _T('-') ) {
            ...
            else if ( argmatch(_T("langid"), ap) ) {
                // Language id: /langid number
                ap = argv[++i];	ASSERT(ap);
                unsigned long u = _tcstoul( ap, NULL, 0 );
                g_params.LangId = (LANGID)u;
                d2print("LANGID specified=%#x lang=%u sublang=%u\n", (LANGID)u, PRIMARYLANGID(u), SUBLANGID(u) );
            }
			else {
...
void showUsage()
{
	...
	dprint(" /va\t- create new version resource\n");
	...
	dprint(" /xi\t- test mode, does not patch the file\n");
	dprint(" /rpdb\t- remove path to .pdb in debug information\n");
    dprint(" /langid number\t- language id for new version resource (use with /va)\n");
	dprint("\n\nExamples:\n");
	dprint("  verpatch d:\\foo.dll 1.2.33.44 /s comment \"it works!\"\n");
